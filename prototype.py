...
output1 = pd.merge(pd.read_csv('continents.csv'), pd.read_csv(
    'countries_unique.csv'), on='Code', how='inner')
...
def main():
    ...
    ps = ''
    widget_venue, widget_count, widget_cont, widget_auth_pos, logtxtbox, start_year, end_year = display_relation()
    if st.button('Submit and Compare'):
        # if(compare == True):
        #logtxt = st.session_state.logtxt
        # logtxtbox.write(logtxt)
        populate_graph(
            widget_venue, widget_count, widget_cont, widget_auth_pos, start_year, end_year)


def populate_graph(venue='', country='', cont='', auth_pos='', start_year='2000', end_year='2022'):
...
def display_relation():
    ...
    with col1:
        widget_cont = st.multiselect(
            'Filter by Continent:', options_Cont, key='Cont')
        widget_venue = st.multiselect(
            'Filter by Conference/Journals:', options_Venue, key='venue')
    with col2:
        widget_count = st.multiselect(
            'Filter by Country:', options_Count, key='country')
    ...
    start_year, end_year = st.slider("Year span:",  min_value=st.session_state.min_max[0], value=[
                                     2000, 2022], max_value=st.session_state.min_max[1])
    # button for clear history
    st.button('Clear History', on_click=clear_multi)
    return(widget_venue, widget_count, widget_cont, widget_auth_pos, logtxtbox, start_year, end_year)
